...
function HttpLogHandler:body_filter(conf)
    ...
    if kong.response.get_header("Content-Encoding") ~= nil  then
        print("Content-Encoding: " .. kong.response.get_header("Content-Encoding"))
        local ctx = kong.ctx.plugin;
        local chunk, eof = ngx.arg[1], ngx.arg[2];
        print("chunk: " .. chunk)
        print("eof: " .. eof)
...
function HttpLogHandler:log(conf)
    ...
    if req_get_body_data() ~= nil then
        -- 适配 text 格式的 body
        log_obj.request.body = req_get_body_data()
    end

    if ctx.response_body ~= nil then
        --log_obj.response.body = cjson_decode(ctx.response_body)
        -- Adapt the response in text format to avoid errors in cjson processing
        -- print("Content-Encoding: " .. ctx.response.get_headers["Content-Encoding"])
    ...
    print("spend time: " .. end_time - begin_time .. " ms")
    q:add(entry)
end
